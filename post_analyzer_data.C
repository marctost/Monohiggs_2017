#define post_analyzer_data_cxx
#include "post_analyzer_data.h"
#include <string>
#include <sstream>
#include <utility>
#include <stdio.h>
...
int main(int argc, const char* argv[]){
	...
	post_analyzer_data t(argv[1], argv[2]);
...
void post_analyzer_data::Loop(Long64_t maxevents, int reportEvery, string SampleName, string save_name, string finalState){
	if (fChain==0) return;
	...
	for (Long64_t jentry=0; jentry<nentriesToCheck;jentry++) {
        ...
        /*
         ...
         fillEvent(10,1,lept_num_1, lept_num_2);
         
         if (pfMET  ) continue;
         fillEvent(11,1,lept_num_1, lept_num_2);
         
         if (higher_vars.inv_mass  ) continue;
         fillEvent(12,1,lept_num_1, lept_num_2);
         */
...
void post_analyzer_data::BookHistos(const char* file2)
{
	...
	h_lep_2_En[i] = new TH1F(("Lepton_2_En"+histname).c_str(), "Lepton_2_En",12,PtBins);h_lep_2_En[i]->Sumw2();
        ...
        h_lep_1_Iso[i]= new TH1F(("Electron iso"+histname).c_str(),"Electron iso",40,0.0,1.0);h_lep_1_Iso[i]->Sumw2();
...
void post_analyzer_data::fillHistos(int histoNumber, double event_weight,int lep_1_index, int lep_2_index, TString final_state)
{
